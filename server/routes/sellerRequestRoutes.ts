  ...
  try {
    ...
    if (existingSeller) {
      ...
      const updatedData = {
        ...
        specialty: specialty?.trim() || 'مشغولات وحرف صعيدية على أصولها',
        governorate: governorate || user.governorate || 'قنا',
        phone: phone.trim(),
        email: email?.trim() || user.email || '',
        bio: bio?.trim() || `ورشة متخصصة في عمل المشغولات الصعيدية والتراثية على أصولها في محافظة ${governorate || user.governorate || 'قنا'}.`,
        story: story?.trim() || 'بدأنا بصنعة أجدادنا وورثناها جيل ورا جيل عشان نوصلكم أحسن ما طلعت إيدين الصعايدة.',
