...
export const GlobalConfirmModal: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-espresso-900 rounded-3xl border border-black/10 dark:border-white/10 max-w-md w-full p-5 sm:p-6 space-y-5 shadow-2xl overflow-hidden relative text-espresso dark:text-cream animate-in zoom-in-95 duration-150"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            <div
              className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${danger
                  ...
                  : 'bg-primary/10 text-primary dark:text-primary-hover border border-primary/20'
                }`}
            >
              {danger ? <Trash2 className="w-5 h-5" /> : <AlertTriangle className="w-5 h-5" />}
            </div>
            <div>
              <h3 className="font-black text-base sm:text-lg text-espresso dark:text-cream">
                {title}
              </h3>
        ...
        {/* Message body */}
        <div className="p-3.5 bg-black/[0.02] dark:bg-cream/[0.03] rounded-2xl border border-black/5 dark:border-white/5">
        ...
        <div className="flex items-center justify-end gap-2.5 pt-1">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer ${danger
                ...
                : 'bg-primary hover:bg-[#855928] text-white'
              } disabled:opacity-60`}
          >
            {isLoading ? (
              <>
