...
export const VisitorMediaGallery: React.FC<
  VisitorMediaGalleryProps
> = ({
  id,
  ...
  className = '',
    ...
    const [localCover, setLocalCover] =
      useState<string | undefined>(
        coverImage
      );

    const [localVideoUrl, setLocalVideoUrl] =
    ...
    const [lightboxZoom, setLightboxZoom] =
      useState(1);
    ...
    const [adminActiveTab, setAdminActiveTab] =
      useState<'gallery' | 'video'>(
        'gallery'
      );
    ...
    const handleSetCover = async (
      ...
      try {
        const res =
          await adminMediaApi.manageEntityGallery(
            userAuth,
            {
              entityType,
              entityId:
        ...
        if (res.success) {
          ...
          addToast(
            'تم التعيين',
            'تم تعيين الصورة كغلاف رئيسي بنجاح',
            'success'
          );
        }
      } catch (err: any) {
    ...
    const handleRemoveFromGallery =
      async (
        ...
        try {
          ...
          addToast(
            'تم الحذف',
            'تم حذف الصورة من المعرض وتحديث التخزين بنجاح',
            'success'
          );
        } catch (err) {
          console.warn(
            'Gallery image delete backend notice:',
            err
          );

          addToast(
            'تم الحذف من المعرض',
            'تمت إزالة الصورة من العرض بنجاح',
            'success'
          );
        } finally {
          setIsProcessing(false);
    ...
    const handleMoveImage =
      async (
        ...
        try {
          ...
          addToast(
            'تم الترتيب',
            'تم تحديث ترتيب صور المعرض بنجاح',
            'success'
          );
        } catch (err) {
          console.warn(
    ...
    // =========================================================
    // NEW IMAGES (SINGLE OR BATCH)
    // =========================================================

    const handleNewImagesUploaded =
      async (
        newUrls: string[]
      ) => {
        ...
        setLocalGallery((prev) => {
          mergedGallery = Array.from(new Set([...prev, ...validUrls]));
          return mergedGallery;
        });

        onGalleryChange?.(mergedGallery);

        if (!localCover && validUrls[0]) {
          setLocalCover(validUrls[0]);
          onCoverChange?.(validUrls[0]);
        }
        ...
        try {
          ...
          addToast(
            'تمت الإضافة',
            validUrls.length === 1
              ? 'تمت إضافة الصورة بنجاح إلى المعرض السحابي'
              : `تمت إضافة ${validUrls.length} صور بنجاح إلى المعرض السحابي`,
            'success'
          );
        } catch (err) {
          console.warn(
            'Backend sync warning on adding image:',
            err
          );

          addToast(
            'تمت الإضافة للمعرض',
            'تم ربط الصور بالمعرض الحالي',
            'success'
          );
        } finally {
          setIsProcessing(false);
    ...
    const handleNewImageUploaded =
      (newUrl: string) => {
        if (newUrl) handleNewImagesUploaded([newUrl]);
      };
      ...
      async (
        ...
        try {
          ...
          if (res.success) {
            addToast(
              'تم الحفظ',
              'تم رفع وتوثيق مقطع الفيديو بنجاح',
              'success'
            );
          }
        } catch (err) {
          console.warn(
            'Backend sync warning on video set:',
            err
          );

          addToast(
            'تم توثيق الفيديو',
            'تم ربط مقطع الفيديو بنجاح',
            'success'
          );
        } finally {
          setIsProcessing(false);
          ...
          >
            ...
            <div className="relative flex min-h-[380px] flex-col items-center justify-center px-5 py-12 text-center sm:px-8">
              ...
              <p className="mt-3 max-w-md text-xs leading-7 text-black/45 dark:text-white/40">
                أضف الصور والفيديوهات التوثيقية
                علشان تبدأ بناء الأرشيف البصري
                الخاص بـ{' '}
                {entityTitle ||
                  'هذا المكان'}.
              </p>
              ...
              <div className="flex w-full items-center gap-2 lg:w-auto">
                ...
                <div className="flex min-w-0 flex-1 items-center rounded-2xl border border-black/10 bg-black/[0.025] p-1 dark:border-white/10 dark:bg-white/[0.025] lg:flex-none">
                  <button
                    ...
                    className={`flex min-w-0 flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-black transition sm:px-4 ${activeTab ===
                      'photos'
                ...
                {isAdmin && (
                  <button
                    ...
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-black/10 bg-white text-black/60 shadow-sm transition hover:border-[#9a6a35]/30 hover:bg-[#9a6a35]/10 hover:text-[#9a6a35] dark:border-white/10 dark:bg-white/[0.03] dark:text-white/60"
                    title="إدارة الوسائط"
                  >
                    ...
                    <div className="relative overflow-hidden bg-[#11100e]">
                      ...
                      <div className="relative flex min-h-[420px] items-center justify-center p-3 sm:min-h-[520px] sm:p-6 lg:min-h-[620px] lg:p-10">
                        {activeImage && (
                          <div className="relative flex max-h-[580px] w-full items-center justify-center">
                            <img
                              ...
                              alt={`${entityTitle || 'المكان'} - صورة ${activeImageIndex +
                                1
                                }`}
                        ...
                        <button
                          ...
                          className="absolute bottom-4 left-4 z-30 flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-black/45 text-white backdrop-blur-xl transition hover:bg-white hover:text-black sm:bottom-7 sm:left-7"
                        ...
                        {localGallery.length >
                          1 && (
                            <button
                              ...
                              className="absolute right-3 top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-xl transition hover:bg-white hover:text-black sm:right-7 sm:h-12 sm:w-12"
                            >
                        ...
                        {localGallery.length >
                          1 && (
                            <button
                              ...
                              className="absolute left-3 top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-xl transition hover:bg-white hover:text-black sm:left-7 sm:h-12 sm:w-12"
                            >
                ...
                ================================================== */}
                          ...
                          (
                            ...
                            return (
                              <button
                                ...
                                className={`group relative h-20 w-28 shrink-0 overflow-hidden rounded-2xl transition sm:h-24 sm:w-36 ${isActive
                    ...
                    <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2 rounded-xl bg-black/5 px-4 py-2.5 text-[10px] font-black text-[#9a6a35] transition hover:bg-[#9a6a35]/10 dark:bg-white/5"
                      >
                        عرض المعرض كاملًا
                          ...
                          (
                            ...
                            return (
                              <button
                                ...
                                className={`group flex w-full items-center gap-3 rounded-2xl p-2 text-right transition ${isSelected
            ...
            >
              ...
              <div className="flex shrink-0 items-center justify-between border-b border-white/10 bg-black/30 px-3 py-3 sm:px-6 sm:py-4">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="hidden h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-white/60 transition hover:bg-white/10 hover:text-white sm:flex"
                    title="تكبير"
                  >
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="hidden h-10 items-center gap-2 rounded-xl bg-red-600/10 px-3 text-xs font-bold text-red-400 transition hover:bg-red-600 hover:text-white sm:flex"
                    >
                  ...
                  <button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 transition hover:bg-red-600"
                    title="إغلاق"
                  >
                    <X className="h-5 w-5" />
                  </button>
              ...
              <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden p-3 sm:p-6">
                <button
                  ...
                  className="absolute right-2 z-30 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-black/55 backdrop-blur-xl transition hover:bg-white hover:text-black sm:right-6 sm:h-12 sm:w-12"
                >
                  <ChevronRight className="h-5 w-5" />
                </button>
                ...
                >
                  <img
                    ...
                    alt={`${entityTitle || 'المكان'} - ${lightboxIndex +
                      1
                      }`}
                ...
                <button
                  ...
                  className="absolute left-2 z-30 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-black/55 backdrop-blur-xl transition hover:bg-white hover:text-black sm:left-6 sm:h-12 sm:w-12"
                >
                  <ChevronLeft className="h-5 w-5" />
                </button>

                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/65 px-4 py-2 text-[8px] text-white/45 backdrop-blur-xl sm:hidden">
              ...
              <div className="flex shrink-0 items-center justify-center gap-2 border-t border-white/10 bg-black/50 px-3 py-3 sm:hidden">
                <button
                  type="button"
                  onClick={() =>
                    setLightboxZoom(
                  ...
                  {localGallery.map(
                    (
                      ...
                      <button
                        ...
                        className={`h-12 w-16 shrink-0 overflow-hidden rounded-lg border-2 transition sm:h-16 sm:w-24 ${idx ===
                          lightboxIndex
              ...
              <div className="flex max-h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-[1.75rem] border border-black/10 bg-[#f5f1eb] shadow-2xl dark:border-white/10 dark:bg-[#171512] sm:rounded-[2rem]">
                ...
                <div className="shrink-0 border-b border-black/10 bg-white px-4 py-4 dark:border-white/10 dark:bg-[#1d1a17] sm:px-7 sm:py-5">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-black/5 text-black/50 transition hover:bg-red-500 hover:text-white dark:bg-white/5 dark:text-white/50"
                    >
                      <X className="h-5 w-5" />
                    </button>
                  </div>
                ...
                <div className="flex shrink-0 border-b border-black/10 bg-white px-3 dark:border-white/10 dark:bg-[#1d1a17] sm:px-7">
                  <button
                    ...
                    className={`flex items-center gap-2 border-b-2 px-4 py-3.5 text-xs font-black transition sm:py-4 ${adminActiveTab ===
                      'gallery'
                                      ...
                                      <div className="mt-2 flex items-center justify-between gap-1">
                                        <div className="flex items-center gap-1">
                                          <button
                                            ...
                                            className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/5 text-black/50 transition hover:bg-black/10 disabled:opacity-20 dark:bg-white/5 dark:text-white/50"
                                            title="تحريك للخلف"
                                          >
                                        ...
                                        <div className="flex items-center gap-1">
                                          {!isCover && (
                                            <button
                                              ...
                                              className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10 text-amber-600 transition hover:bg-amber-500/20"
                                              title="تعيين كغلاف"
                                            >
                                          ...
                                          <button
                                            ...
                                            className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500/10 text-red-600 transition hover:bg-red-500/20"
                                            title="حذف"
                                          >
                          ...
                          <AdminMediaUploader
                            ...
                            onChange={(
                              ...
                              if (
                                ...
                                handleNewImagesUploaded(
                                  urls
                                );
                              }
                            }}
                            helperText="سيتم رفع الصور وربطها مباشرة بمعرض المكان."
                          />
                        </div>
                      </div>
                    )}
                                ...
                                (
                                  ...
                                  >
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        setVideoPendingDelete(
                                          vidUrl
                                        )
                                      }
                                      className="flex items-center justify-center gap-2 rounded-xl bg-red-500/10 px-4 py-2.5 text-xs font-bold text-red-600 transition hover:bg-red-500/20"
                                    >
                                      <Trash2 className="h-4 w-4" />
                                      إزالة
                                    </button>
                          ...
                          <AdminMediaUploader
                            ...
                            onChange={(
                              ...
                              if (
                                ...
                                handleNewVideoUploaded(
                                  finalVidUrl
                                );
                              }
                            }}
                            helperText="رفع فيديو توثيقي وربطه بالمكان."
                          />
                        </div>
                      </div>
                    )}
                ...
                <div className="flex shrink-0 items-center justify-between border-t border-black/10 bg-white px-4 py-4 dark:border-white/10 dark:bg-[#1d1a17] sm:px-7">
                  ...
                  <button
                    ...
                    className="mr-auto rounded-xl bg-[#9a6a35] px-5 py-2.5 text-xs font-black text-white transition hover:bg-[#83592c]"
                  >
                    إغلاق
                  </button>
                </div>
              ...
              <div className="p-5 sm:p-6">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-xl bg-black/5 px-4 py-3 text-xs font-bold dark:bg-white/5"
                  >
                    إلغاء
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-600 px-4 py-3 text-xs font-black text-white"
                  >
                    {isProcessing ? (
            ...
            <div className="w-full max-w-md rounded-[2rem] border border-black/10 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#211d19]">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-black/40 dark:text-white/40">
                    سيتم إزالة الفيديو من قائمة المكان.
                  </p>
                </div>
              </div>
