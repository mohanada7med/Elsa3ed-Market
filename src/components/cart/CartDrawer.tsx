    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 left-0 max-w-full flex pl-0 sm:pl-10 w-full sm:w-auto">
        <motion.div
          ...
          className="w-full sm:w-[440px] max-w-full bg-[#eee8dc]/95 dark:bg-[#0b0b0a]/95 backdrop-blur-2xl border-r border-black/10 dark:border-white/10 shadow-2xl flex flex-col h-full text-[#211d18] dark:text-[#f5f0e7]"
        >
          {/* Drawer Header */}
          <div className="p-4 sm:p-5 bg-white/60 dark:bg-[#151513]/60 backdrop-blur-md border-b border-black/10 dark:border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-[#9a6a35]/15 text-[#9a6a35] dark:text-[#d5a56d] flex items-center justify-center shrink-0">
                <ShoppingBag className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-bold text-[#211d18] dark:text-[#f5f0e7] text-base">سلة المشتريات التراثية</h3>
            ...
            <button
              ...
              onClick={() => setIsCartDrawerOpen(false)}
              className="p-2 rounded-xl text-black/50 hover:text-black dark:text-white/50 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 transition-colors min-h-[40px] min-w-[40px] flex items-center justify-center cursor-pointer"
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            {cart.length === 0 ? (
              <div className="text-center py-16 px-4">
                <div className="w-20 h-20 rounded-full bg-[#9a6a35]/15 text-[#9a6a35] dark:text-[#d5a56d] flex items-center justify-center mx-auto mb-4 border border-[#9a6a35]/30">
                  <ShoppingBag className="w-10 h-10 opacity-75" />
                </div>
                <h4 className="font-bold text-[#211d18] dark:text-[#f5f0e7] text-lg">سلة المشتريات فارغة</h4>
              ...
              cart.map((item, idx) => {
                ...
                return (
                  <div
                    key={prodId}
                    id={`cart-item-${prodId}`}
                    className="p-3.5 bg-white/75 dark:bg-[#151513]/90 rounded-2xl border border-black/10 dark:border-white/10 shadow-xs flex gap-3 relative group"
                  >
                    <img
                      src={img}
                      alt={title}
