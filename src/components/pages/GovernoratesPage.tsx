      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2 text-xs sm:text-sm text-[#73675B] dark:text-[#9C8F82]">
            ...
            <span>/</span>
            <span className="text-[#B24C2B] font-bold">محافظات صعيد مصر</span>
          </div>
        ...
        <div className="relative text-center max-w-4xl mx-auto mb-8 sm:mb-12 p-6 sm:p-10 rounded-3xl overflow-hidden bg-[#FAF7F2] dark:bg-[#1C1714] border-2 border-amber-300/60 dark:border-amber-900/40 shadow-sm">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black text-[#241E1A] dark:text-[#FAF6F2] font-heritage tracking-tight mb-3">
              محافظات صعيد مصر
            </h1>
            <p className="text-sm sm:text-base text-[#73675B] dark:text-[#A89C90] leading-relaxed max-w-2xl mx-auto">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-80 rounded-3xl bg-white dark:bg-[#1E1917] border border-[#E5DDD3] dark:border-[#352B24] animate-pulse"
              />
            ))}
          </div>
        ) : filteredGovernorates.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-[#1E1917] rounded-3xl border border-[#E5DDD3] dark:border-[#352B24]">
            <Compass className="w-12 h-12 text-[#73675B] mx-auto mb-3" />
            <h3 className="text-lg font-bold text-[#241E1A] dark:text-[#FAF6F2]">لم يتم العثور على نتائج</h3>
            <p className="text-xs text-[#73675B] mt-1">جرب البحث بكلمات أخرى أو اختر إقليم مختلف</p>
          </div>
        ) : (
            ...
            {filteredGovernorates.map((gov) => {
              ...
              return (
                <div
                  ...
                  className="group bg-white dark:bg-[#1E1917] rounded-3xl border border-[#E5DDD3] dark:border-[#352B24] hover:border-[#B24C2B] overflow-hidden shadow-xs hover:shadow-lg transition-all duration-300 flex flex-col justify-between cursor-pointer"
                >
                  {/* Image Cover & Badges */}
                  <div className="relative aspect-[16/10] overflow-hidden bg-[#E5DDD3] dark:bg-[#26201B]">
                    <img
                  ...
                  <div className="relative p-5 sm:p-6 flex-1 flex flex-col justify-between overflow-hidden">
                    ...
                    <div className="relative z-10">
                      {/* Short Intro */}
                      <p className="text-xs sm:text-sm text-[#73675B] dark:text-[#A89C90] leading-relaxed mb-4 line-clamp-2">
                        {gov.shortIntro}
                      </p>
                      ...
                      {gov.famousFor && gov.famousFor.length > 0 && (
                        <div className="mb-4">
                          <span className="text-[11px] font-bold text-[#73675B] dark:text-[#9C8F82] block mb-2">
                            تشتهر بـ:
                          </span>
                          <div className="flex flex-wrap gap-1.5">
                            {gov.famousFor.slice(0, 3).map((item, idx) => (
                              <span
                                key={idx}
                                className="text-[11px] font-medium bg-[#FAF7F2] dark:bg-[#26201B] text-[#241E1A] dark:text-[#FAF6F2] px-2.5 py-1 rounded-lg border border-[#E5DDD3] dark:border-[#352B24]"
                              >
                                {item}
                              </span>
                            ))}
                            {gov.famousFor.length > 3 && (
                              <span className="text-[10px] text-[#73675B] self-center">
                    ...
                    <div className="pt-4 border-t border-[#F0EAE1] dark:border-[#2D2622] flex items-center justify-between text-[#B24C2B] dark:text-[#FF855D] font-bold text-xs sm:text-sm group-hover:text-[#9E4F36]">
