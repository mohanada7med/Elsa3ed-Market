...
export const SellersDirectoryPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="rounded-[2rem] bg-espresso text-white dark:bg-cream dark:text-black p-6 sm:p-10 shadow-xl relative overflow-hidden border border-black/10 dark:border-white/10 backdrop-blur-xl">
        <div className="relative z-10 max-w-2xl space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 dark:bg-black/10 text-[#d5a56d] dark:text-primary text-xs font-bold backdrop-blur-xs">
          ...
          <div className="pt-2">
            <div className="relative max-w-md">
              <input
                ...
                className="w-full pl-4 pr-10 py-3 bg-white dark:bg-espresso-900 text-espresso dark:text-cream rounded-xl text-xs sm:text-sm outline-none shadow-md placeholder:text-black/40 dark:placeholder:text-white/40 min-h-[44px] border border-black/10 dark:border-white/10 focus:border-primary transition-colors"
              />
        ...
        {GOVERNORATES.map((gov) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all shrink-0 min-h-[40px] cursor-pointer ${selectedGov === gov
              ...
              : 'bg-white/75 dark:bg-espresso-900/90 text-black/75 dark:text-white/75 hover:bg-black/5 dark:hover:bg-white/10 border border-black/10 dark:border-white/10 backdrop-blur-xl'
              }`}
          >
      ...
      {filteredSellers.length === 0 ? (
        <div className="bg-white/75 dark:bg-espresso-900/90 rounded-[2rem] border border-black/10 dark:border-white/10 p-12 text-center my-6 space-y-4 shadow-lg backdrop-blur-xl">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-espresso text-white dark:bg-cream dark:text-black hover:bg-primary dark:hover:bg-primary-hover text-xs font-bold rounded-xl shadow-xs transition-colors cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filteredSellers.map((seller) => (
            <div
              ...
              className="group relative overflow-hidden rounded-[2rem] bg-white/75 dark:bg-espresso-900/90 border border-black/10 dark:border-white/10 cursor-pointer transition-all duration-500 hover:-translate-y-1.5 hover:border-primary hover:shadow-[0_20px_50px_rgba(154,106,53,0.12)] flex flex-col justify-between backdrop-blur-xl shadow-lg"
            >
              {/* Cover Banner */}
              <div className="relative h-36 w-full overflow-hidden bg-black/5 dark:bg-cream/5">
                ...
                <div className="absolute top-3 right-3">
                  <span className="bg-primary text-white text-[10px] font-bold px-2.5 py-1 rounded-xl shadow-xs flex items-center gap-1">
              ...
              <div className="p-5 pt-0 flex-1 flex flex-col justify-between relative">
                <div>
                  <div className="relative -mt-10 mb-3 flex items-end justify-between">
                    ...
                    <div className="flex items-center gap-1 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10 px-2.5 py-1 rounded-xl text-xs font-bold backdrop-blur-md">
                  ...
                  <h3 className="font-bold text-lg group-hover:text-primary transition-colors leading-tight font-serif">
                    {seller.brandName}
                  </h3>
                ...
                <div className="mt-4 pt-3 border-t border-black/10 dark:border-white/10 flex items-center justify-between text-xs">
                  <span className="text-primary font-bold bg-primary/10 px-2.5 py-1 rounded-lg text-[11px] border border-primary/30">
                    {seller.specialty}
                  </span>
