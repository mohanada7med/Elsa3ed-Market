...
export const CheckoutPage: React.FC = () => {
  ...
  if (cart.length === 0 && !completedOrder) {
    return (
      ...
      >
        <div className="max-w-md w-full p-8 sm:p-10 text-center space-y-6 bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 shadow-xl">
          <div className="w-20 h-20 rounded-2xl bg-primary/10 text-primary dark:text-primary-hover flex items-center justify-center mx-auto shadow-inner">
          ...
          <button
            ...
            className="w-full py-4 bg-espresso text-white dark:bg-cream dark:text-black hover:bg-primary dark:hover:bg-primary-hover text-xs sm:text-sm font-black rounded-[1.25rem] shadow-lg cursor-pointer transition-all duration-300 hover:scale-[1.01]"
          >
  ...
  if (completedOrder) {
    ...
    return (
      <div
        dir="rtl"
        className="min-h-screen bg-cream text-espresso dark:bg-espresso-900 dark:text-cream max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-12 py-12 space-y-8"
      >
        <div className="max-w-3xl mx-auto bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 shadow-xl p-6 sm:p-10 text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-espresso/70 dark:text-cream/70">
              رقم الطلب الخاص بك:{' '}
              <span className="font-mono font-bold text-primary dark:text-primary-hover text-base">
          ...
          {isManualTransfer && (
            <div className="p-4 rounded-[1.5rem] bg-amber-500/10 border border-amber-500/20 text-right space-y-2">
              <div className="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-bold text-xs">
                <Clock className="w-4 h-4 text-amber-600 dark:text-primary-hover" />
                <span>حالة الدفع: قيد مراجعة وتأكيد التحويل من الإدارة</span>
              </div>
              <p className="text-xs text-espresso/80 dark:text-cream/80 leading-relaxed">
            ...
            <div className="bg-sky-500/10 border border-sky-500/20 p-5 rounded-[1.5rem] text-right text-xs space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-black/5 dark:bg-cream/10 hover:bg-sky-500/20 text-sky-800 dark:text-sky-300 border border-sky-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                >
              ...
              <div className="bg-black/5 dark:bg-cream/5 p-3 rounded-xl border border-black/5 dark:border-white/5 flex items-center justify-between">
                <span className="text-espresso/70 dark:text-cream/70">معرف الدفع (IPA):</span>
                <strong className="font-mono text-sm text-sky-800 dark:text-sky-300 select-all" dir="ltr">
                  {paymentConfig.instaPayAccount}
                </strong>
              </div>
              <p className="text-[11px] text-espresso/80 dark:text-cream/80">
                المبلغ المطلوب تحويله: <strong className="font-black text-primary dark:text-primary-hover">{completedOrder.total} ج.م</strong>.
              </p>
            </div>
          )}
            ...
            <div className="bg-rose-500/10 border border-rose-500/20 p-5 rounded-[1.5rem] text-right text-xs space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-black/5 dark:bg-cream/10 hover:bg-rose-500/20 text-rose-800 dark:text-rose-300 border border-rose-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                >
              ...
              <div className="bg-black/5 dark:bg-cream/5 p-3 rounded-xl border border-black/5 dark:border-white/5 flex items-center justify-between">
                <span className="text-espresso/70 dark:text-cream/70">رقم المحفظة المعتمد:</span>
          ...
          {completedOrder.paymentMethod === 'cod' && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-[1.5rem] text-right text-xs space-y-2">
              ...
              <p className="text-xs text-espresso/80 dark:text-cream/80">
                المبلغ المستحق عند التسليم: <strong className="font-black text-primary dark:text-primary-hover">{completedOrder.total} ج.م</strong>. سيقوم مندوب الشحن بالتواصل معك قبل التوصيل مع إمكانية فحص سلامة التغليف قبل السداد.
              </p>
            </div>
          )}

          {/* Order Summary Details */}
          <div className="bg-black/5 dark:bg-cream/5 p-5 sm:p-6 rounded-[1.5rem] border border-black/10 dark:border-white/10 text-right space-y-3">
            <h4 className="font-bold text-xs text-espresso dark:text-cream border-b border-black/10 dark:border-white/10 pb-2">
                  ...
                  <div key={prodId} className="py-2.5 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-espresso/60 dark:text-cream/60">الكمية: {qty} • الورشة: {sellerName}</span>
                      </div>
                    </div>
                    <span className="font-bold text-primary dark:text-primary-hover">{price * qty} ج.م</span>
                  </div>
                );
              })}
            </div>

            <div className="pt-3 border-t border-black/10 dark:border-white/10 flex justify-between text-sm font-black text-espresso dark:text-cream">
              <span>إجمالي الفاتورة المطلوب:</span>
              <span className="text-primary dark:text-primary-hover text-base font-black">{completedOrder.total} ج.م</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            <button
              ...
              className="px-6 py-3.5 bg-espresso text-white dark:bg-cream dark:text-black hover:bg-primary dark:hover:bg-primary-hover text-xs font-black rounded-[1.25rem] shadow-lg transition-all hover:scale-[1.02] flex items-center gap-2 cursor-pointer"
            >
            ...
            <button
              ...
              onClick={() => setActivePage('products')}
              className="px-6 py-3.5 bg-black/5 dark:bg-cream/5 hover:bg-black/10 dark:hover:bg-white/10 text-espresso dark:text-cream border border-black/10 dark:border-white/10 text-xs font-bold rounded-[1.25rem] transition-colors cursor-pointer"
            >
              ارجع للسوق
            </button>
          </div>
  ...
  return (
    <div
      dir="rtl"
      className="min-h-screen bg-cream text-espresso dark:bg-espresso-900 dark:text-cream max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 py-6 sm:py-8 pb-20 sm:pb-12 space-y-6 sm:space-y-8"
    >
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-xs text-espresso/60 dark:text-cream/60 font-medium">
        ...
        <ChevronRight className="w-3.5 h-3.5 rotate-180 opacity-50" />
        <span className="text-espresso dark:text-cream font-bold">إتمام الطلب والدفع</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7">
          <form onSubmit={handlePlaceOrder} className="space-y-6">
            ...
            <div className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 p-6 sm:p-8 shadow-lg space-y-5">
              <div className="flex items-center gap-3 border-b border-black/10 dark:border-white/10 pb-4">
                <div className="w-9 h-9 rounded-xl bg-primary/15 text-primary dark:text-primary-hover flex items-center justify-center font-black text-sm">
                  1
                </div>
                <div>
                  <h3 className="font-black font-serif text-espresso dark:text-cream text-lg">عنوان الشحن والتوصيل في مصر</h3>
                  <p className="text-[11px] text-espresso/60 dark:text-cream/60">يرجى كتابة العنوان بدقة لضمان سرعة التوصيل</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">الاسم بالكامل *</label>
                  <input
                    type="text"
                    required
                    value={fullName}
                    ...
                    className="w-full px-4 py-3 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10 text-espresso dark:text-cream rounded-[1rem] text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 min-h-[44px]"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">رقم الهاتف للتواصل *</label>
                  <input
                    type="tel"
                    required
                    value={phone}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">المحافظة *</label>
                  <select
                    value={governorate}
                    onChange={(e) => setGovernorate(e.target.value as Governorate)}
                    className="w-full px-4 py-3 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10 text-espresso dark:text-cream rounded-[1rem] text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 min-h-[44px] cursor-pointer"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">المدينة / الحي / القرية *</label>
                  <input
                    type="text"
                    required
                    value={city}
              ...
              <div>
                <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">العنوان التفصيلي (الشارع، رقم المبنى، الشقة) *</label>
                <input
                  type="text"
                  required
                  value={address}
              ...
              <div>
                <label className="block text-xs font-bold text-espresso dark:text-cream mb-1.5">ملاحظات إضافية للتوصيل والتغليف (اختياري)</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="مثال: يرجى الاتصال قبل الوصول بنصف ساعة، القطعة هدية تغليف خاص..."
                  rows={2}
                  className="w-full px-4 py-3 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10 text-espresso dark:text-cream rounded-[1rem] text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
                />
              </div>
            </div>
            ...
            <div className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 p-6 sm:p-8 shadow-lg space-y-5">
              <div className="flex items-center gap-3 border-b border-black/10 dark:border-white/10 pb-4">
                ...
                <div>
                  <h3 className="font-black font-serif text-espresso dark:text-cream text-lg">طريقة الدفع المعتمدة</h3>
                  <p className="text-[11px] text-espresso/60 dark:text-cream/60">اختر وسيلة الدفع المناسبة لتحويل قيمة الطلب</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <label
                  className={`p-4 rounded-[1.5rem] border flex flex-col gap-3 cursor-pointer transition-all duration-300 ${paymentMethod === 'instapay'
                    ...
                    : 'border-black/10 dark:border-white/10 bg-black/5 dark:bg-cream/5 hover:bg-black/10 dark:hover:bg-white/10'
                    }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-espresso/60 dark:text-cream/60">
                  ...
                  {paymentMethod === 'instapay' && (
                    <div className="p-4 bg-black/5 dark:bg-cream/5 rounded-xl border border-black/10 dark:border-white/10 text-xs space-y-3 animate-fadeIn">
                      <div className="flex items-center justify-between bg-sky-500/10 p-3 rounded-lg border border-sky-500/20">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-white dark:bg-espresso-900 hover:bg-sky-500/20 text-sky-800 dark:text-sky-300 border border-sky-500/30 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer min-h-[36px]"
                        >
                      ...
                      <div className="text-[11px] text-espresso/70 dark:text-cream/70 space-y-1">
                        <p>1. افتح تطبيق إنستاباي وقم بتحويل مبلغ <strong className="text-primary dark:text-primary-hover font-bold">{cartTotal} ج.م</strong> إلى المعرف الموضح أعلاه.</p>
                        <p>2. أدخل معرف حسابك أو الرقم المرجعي للتحويل بالأسفل لتسريع عملية التأكيد.</p>
                      </div>

                      <div className="pt-1">
                        <label className="block text-[11px] font-bold text-espresso dark:text-cream mb-1">
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2.5 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10 text-espresso dark:text-cream rounded-lg text-xs outline-none focus:border-primary"
                        />
                      </div>
                    </div>
                  )}
                    ...
                    <div className="p-4 bg-black/5 dark:bg-cream/5 rounded-xl border border-black/10 dark:border-white/10 text-xs space-y-3 animate-fadeIn">
                      <div className="flex items-center justify-between bg-rose-500/10 p-3 rounded-lg border border-rose-500/20">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-white dark:bg-espresso-900 hover:bg-rose-500/20 text-rose-800 dark:text-rose-300 border border-rose-500/30 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer min-h-[36px]"
                        >
                      ...
                      <div className="text-[11px] text-espresso/70 dark:text-cream/70 space-y-1">
                        <p>1. قم بتحويل مبلغ <strong className="text-primary dark:text-primary-hover font-bold">{cartTotal} ج.م</strong> إلى رقم فودافون كاش الموضح أعلاه.</p>
                ...
                >
                  ...
                  {paymentMethod === 'cod' && (
                    <div className="p-3 bg-black/5 dark:bg-cream/5 rounded-xl border border-black/10 dark:border-white/10 text-xs text-espresso/70 dark:text-cream/70 space-y-1 animate-fadeIn">
              ...
              <div className="p-3.5 bg-black/5 dark:bg-cream/5 rounded-2xl border border-black/10 dark:border-white/10 text-[11px] text-espresso/70 dark:text-cream/70 flex items-start gap-2.5">
            ...
            <button
              ...
              className={`w-full py-4 bg-espresso text-white dark:bg-cream dark:text-black hover:bg-primary dark:hover:bg-primary-hover font-black text-sm rounded-[1.25rem] shadow-xl flex items-center justify-center gap-2 transition-all hover:scale-[1.01] cursor-pointer min-h-[50px] ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''
                }`}
            >
              {isSubmitting ? (
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 p-6 sm:p-7 shadow-lg space-y-5">
            <h3 className="font-black font-serif text-espresso dark:text-cream text-base border-b border-black/10 dark:border-white/10 pb-3">
                ...
                return (
                  <div key={prodId} className="pt-2.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold text-espresso dark:text-cream truncate">{title}</h4>
                      <span className="text-[10px] text-espresso/60 dark:text-cream/60 block">
                        الكمية: {qty} • {sellerGov}
                      </span>
                      <span className="text-xs font-black text-primary dark:text-primary-hover block">{price * qty} ج.م</span>
                    </div>
                  </div>
                );
              })}
            ...
            {/* Price Calculations */}
            <div className="pt-3 border-t border-black/10 dark:border-white/10 space-y-2 text-xs text-espresso/70 dark:text-cream/70">
              <div className="flex justify-between">
                <span>المجموع الفرعي:</span>
                <span className="font-bold text-espresso dark:text-cream">{cartSubtotal} ج.م</span>
              </div>
              <div className="flex justify-between">
                <span>تكلفة الشحن والتغليف:</span>
                <span className="font-bold text-espresso dark:text-cream">
              ...
              <div className="pt-3 border-t border-black/10 dark:border-white/10 flex justify-between text-base font-black text-espresso dark:text-cream">
                <span>الإجمالي المطلوب:</span>
                <span className="text-xl text-primary dark:text-primary-hover font-black">{cartTotal} ج.م</span>
              </div>
            </div>
