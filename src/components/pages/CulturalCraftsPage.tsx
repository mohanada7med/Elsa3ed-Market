      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2 text-xs sm:text-sm text-[#73675B] dark:text-[#9C8F82]">
            ...
            <span>/</span>
            <span className="text-[#B24C2B] font-bold">موسوعة الحرف والورش التراثية</span>
          </div>
        ...
        {/* Filter Bar */}
        <div className="bg-white dark:bg-[#1E1917] rounded-2xl p-4 sm:p-5 border border-[#E5DDD3] dark:border-[#352B24] shadow-xs mb-8 space-y-4">
          ...
          {governorates.length > 0 && (
            <div className="flex items-center gap-2 pt-3 border-t border-[#F0EAE1] dark:border-[#2D2622] overflow-x-auto no-scrollbar">
              <span className="text-xs font-bold text-[#73675B] shrink-0">المحافظة:</span>
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors shrink-0 ${
                  governorateFilter === 'all'
                    ? 'bg-[#B24C2B] text-white'
                    : 'bg-[#FAF7F2] dark:bg-[#26201B] text-[#73675B]'
                }`}
              >
                الكل
              </button>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors shrink-0 ${
                    governorateFilter === gov
                      ? 'bg-[#B24C2B] text-white'
                      : 'bg-[#FAF7F2] dark:bg-[#26201B] text-[#73675B] hover:bg-[#E5DDD3]'
                  }`}
                >
                  {gov}
                </button>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-80 rounded-3xl bg-white dark:bg-[#1E1917] border border-[#E5DDD3] animate-pulse"
              />
            ))}
          </div>
          ...
          <div className="text-center py-16 bg-white dark:bg-[#1E1917] rounded-3xl border border-[#E5DDD3] dark:border-[#352B24]">
            ...
            <p className="text-xs text-[#73675B] mt-1">جرب تغيير شروط البحث أو الفلتر</p>
          </div>
        ) : (
            ...
            {filteredCrafts.map((craft) => (
              ...
              >
                ...
                <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <p className="text-xs sm:text-sm text-[#73675B] dark:text-[#A89C90] leading-relaxed mb-4 line-clamp-3">
                    ...
                    {craft.materials && craft.materials.length > 0 && (
                      <div className="mb-4">
                        <span className="text-[11px] font-bold text-[#73675B] block mb-1.5">
                          الخامات الطبيعية:
                        </span>
                        <div className="flex flex-wrap gap-1.5">
                          {craft.materials.slice(0, 3).map((mat, i) => (
                            <span
                              key={i}
                              className="text-[11px] bg-[#FAF7F2] dark:bg-[#26201B] text-[#241E1A] dark:text-[#FAF6F2] px-2.5 py-0.5 rounded-lg border border-[#E5DDD3] dark:border-[#352B24]"
                            >
                              {mat}
                            </span>
                          ))}
