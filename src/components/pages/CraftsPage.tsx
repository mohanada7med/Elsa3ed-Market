...
export const CraftsPage: React.FC = () => {
  ...
  return (
    <div
      dir="rtl"
      className="min-h-screen bg-cream text-espresso dark:bg-espresso-900 dark:text-cream max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-12 py-8 space-y-10"
    >
      {/* Breadcrumb */}
        ...
        <div className="relative z-10 max-w-3xl space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-6 py-3.5 rounded-[1.25rem] bg-primary hover:bg-primary-hover text-white font-black text-xs shadow-lg flex items-center gap-2 transition-all cursor-pointer hover:scale-[1.02]"
            >
      ...
      {/* Filter & Search Bar */}
      <div className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[1.5rem] border border-black/10 dark:border-white/10 p-4 sm:p-5 shadow-lg space-y-4 transition-colors">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          {/* Search Input */}
          <div className="relative flex-1">
            <Search className="w-4 h-4 text-espresso/50 dark:text-cream/50 absolute right-3.5 top-1/2 -translate-y-1/2" />
            <input
              type="text"
              value={searchQuery}
            ...
            {searchQuery && (
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-espresso/60 dark:text-cream/60 hover:text-primary cursor-pointer"
              >
                مسح
              </button>
            )}
          ...
          {/* Results count badge */}
          <div className="text-xs font-bold text-espresso/80 dark:text-cream/80 px-4 py-2.5 bg-black/5 dark:bg-cream/5 rounded-[1rem] border border-black/10 dark:border-white/10 shrink-0 self-start md:self-auto flex items-center gap-1.5">
            <BookOpen className="w-3.5 h-3.5 text-primary dark:text-primary-hover" />
            <span>
              {isLoading
                ? 'جاري التحميل...'
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
          <span className="text-espresso/60 dark:text-cream/60 font-bold shrink-0 ml-1">المحافظة:</span>
          {UPPER_EGYPT_GOVS.map((gov) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl font-bold transition-all shrink-0 cursor-pointer ${selectedGov === gov
                ...
                : 'bg-black/5 dark:bg-cream/5 text-espresso/80 dark:text-cream/80 hover:bg-black/10 dark:hover:bg-white/10 border border-black/10 dark:border-white/10'
                }`}
            >
              {gov}
            </button>
      ...
      {isLoading ? (
        <div className="space-y-6">
          {[1, 2].map((n) => (
            <div
              key={n}
              className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 p-6 sm:p-10 animate-pulse space-y-6"
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-5 h-64 bg-black/5 dark:bg-cream/5 rounded-2xl" />
                <div className="lg:col-span-7 space-y-4">
                  <div className="h-6 w-32 bg-black/5 dark:bg-cream/5 rounded-lg" />
                  <div className="h-8 w-3/4 bg-black/5 dark:bg-cream/5 rounded-lg" />
                  <div className="h-20 w-full bg-black/5 dark:bg-cream/5 rounded-lg" />
                  <div className="grid grid-cols-2 gap-2">
                    <div className="h-10 bg-black/5 dark:bg-cream/5 rounded-xl" />
                    <div className="h-10 bg-black/5 dark:bg-cream/5 rounded-xl" />
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-dashed border-black/20 dark:border-white/20 p-12 text-center max-w-2xl mx-auto space-y-4 shadow-lg">
          <div className="w-16 h-16 rounded-2xl bg-primary/10 text-primary dark:text-primary-hover flex items-center justify-center mx-auto border border-primary/20">
            <BookOpen className="w-8 h-8" />
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-espresso dark:text-cream font-serif">
            ملقيناش حرف مطابقة
          </h2>
          <p className="text-xs sm:text-sm text-espresso/70 dark:text-cream/70 leading-relaxed max-w-md mx-auto">
          ...
          {(selectedGov !== 'الكل' || searchQuery) && (
            <button
              ...
              className="px-4 py-2 bg-black/5 dark:bg-cream/5 hover:bg-black/10 dark:hover:bg-white/10 text-espresso dark:text-cream text-xs font-bold rounded-xl border border-black/10 dark:border-white/10 inline-flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          {filteredCrafts.map((story, idx) => {
            ...
            return (
              <motion.article
                ...
                className="bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl rounded-[2rem] border border-black/10 dark:border-white/10 shadow-lg overflow-hidden p-6 sm:p-10 transition-colors"
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
                  {/* Visual Side */}
                  <div className={`lg:col-span-5 ${idx % 2 === 1 ? 'lg:order-2' : ''}`}>
                    <div className="relative rounded-2xl overflow-hidden shadow-lg aspect-4/3 bg-black/5 dark:bg-cream/5 border border-black/10 dark:border-white/10">
                      {story.image ? (
                        <img
                          src={story.image}
                          alt={story.title}
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center bg-black/5 dark:bg-cream/5 text-primary dark:text-primary-hover p-6 text-center">
                          <BookOpen className="w-12 h-12 opacity-60 mb-2" />
                          <span className="text-xs font-bold">{story.title}</span>
                          <span className="text-[10px] text-espresso/50 dark:text-cream/50 mt-1">{story.governorate}</span>
                        </div>
                      )}
                  ...
                  <div className={`lg:col-span-7 space-y-4 ${idx % 2 === 1 ? 'lg:order-1' : ''}`}>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="px-3 py-1 bg-primary/15 text-primary dark:text-primary-hover text-xs font-bold rounded-lg border border-primary/30 inline-flex items-center gap-1">
                        <MapPin className="w-3 h-3" />
                        <span>{story.governorate}</span>
                      </span>

                      {story.city && (
                        <span className="px-2.5 py-1 bg-black/5 dark:bg-cream/5 text-espresso dark:text-cream text-xs font-semibold rounded-lg border border-black/10 dark:border-white/10">
                          {story.city}
                        </span>
                      )}
                      ...
                      {story.historyAge && (
                        <span className="text-xs text-espresso/50 dark:text-cream/50 font-medium">| {story.historyAge}</span>
                      )}
                    </div>

                    <div>
                      <h2 className="text-2xl sm:text-3xl font-black text-espresso dark:text-cream font-serif">
                        {story.title}
                      </h2>
                      {story.subtitle && (
                        <p className="text-sm font-bold text-primary dark:text-primary-hover mt-1">{story.subtitle}</p>
                      )}
                    </div>

                    <p className="text-xs sm:text-sm text-espresso/80 dark:text-cream/80 leading-relaxed whitespace-pre-line">
                      {story.description}
                    </p>
                    ...
                    {story.materials && story.materials.length > 0 && (
                      <div className="pt-1">
                        <h4 className="text-xs font-bold text-primary dark:text-primary-hover mb-1.5 flex items-center gap-1">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {story.materials.map((mat, mIdx) => (
                            <span
                              key={mIdx}
                              className="px-2.5 py-1 bg-black/5 dark:bg-cream/5 text-espresso dark:text-cream text-[11px] font-medium rounded-lg border border-black/10 dark:border-white/10"
                            >
                              {mat}
                            </span>
                          ))}
                    ...
                    {stepsOrTechniques.length > 0 && (
                      <div className="space-y-2 pt-2">
                        <h4 className="text-xs font-bold text-primary dark:text-primary-hover">أسرار ومراحل الصنعة المتوارثة:</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {stepsOrTechniques.map((step, sIdx) => (
                            <div
                              key={sIdx}
                              className="p-3 bg-black/5 dark:bg-cream/5 rounded-xl border border-black/10 dark:border-white/10 text-[11px] text-espresso dark:text-cream flex items-start gap-2"
                            >
                    ...
                    {story.sources && story.sources.length > 0 && (
                      <div className="pt-2 text-[11px] text-espresso/60 dark:text-cream/60 border-t border-black/10 dark:border-white/10">
                        <span className="font-bold text-espresso dark:text-cream ml-1">المراجع والمصادر:</span>
                            ...
                            <span key={srcIdx} className="inline-flex items-center gap-1">
                              {src.sourceUrl ? (
                                <a
                                  ...
                                  className="text-primary dark:text-primary-hover hover:underline inline-flex items-center gap-0.5"
                                >
