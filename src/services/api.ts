...
function getAuthHeadersRaw(user?: { id?: string; role?: string; sellerId?: string }): HeadersInit {
  const headers: Record<string, string> = {};
  const token = getStoredToken();
  if (token) {
    headers['Authorization'] = `Bearer ${token}`;
...
export const api = {
  ...
  // استرجاع صور المعرض التراثي السحابي لورش وحرف الصعيد (فخار، تلي، نول ونسيج، خوص، ألاباستر)
  async fetchCloudinaryImages(craftFilter?: string): Promise<Array<{ id?: string; title?: string; url: string; secure_url?: string; public_id?: string; region?: string; craft?: string; craftCategory?: string }>> {
    ...
    // فلترة تأكيدية للورش فقط واستبعاد أي صورة لا تخص الورش
    const isWorkshopItem = (item: any): boolean => {
      const text = `${item.public_id || ''} ${item.title || ''} ${item.craft || ''}`.toLowerCase();
      ...
      const valid = ['pottery', 'fokhar', 'fukhar', 'clay', 'tally', 'telli', 'weaving', 'loom', 'kilim', 'khous', 'palm', 'alabaster', 'wood', 'copper', 'craft', 'workshop', 'فخار', 'تلي', 'نول', 'نسيج', 'خوص', 'كليم', 'ورش', 'حرفة', 'صنعة', 'ألاباستر'];
      return valid.some((ok) => text.includes(ok));
    };

    try {
      const url = craftFilter && craftFilter !== 'all'
        ? `/api/cloudinary/images?craft=${encodeURIComponent(craftFilter)}`
        : '/api/cloudinary/images';
      const response = await fetch(url);
      if (!response.ok) {
        return fallbackImages.filter(isWorkshopItem);
      }
      ...
      // تصفية حصرية لصور الورش فقط
      const filtered = candidates.filter(isWorkshopItem);
  ...
  uploadReelVideoWithProgress(options: {
    ...
    onProgress?: (info: {
      ...
      total: number;
      percentage: number;
      state: 'uploading' | 'uploaded' | 'processing';
      currentChunk?: number;
      totalChunks?: number;
    }) => void;
    onCancelRef?: (cancelFn: () => void) => void;
    targetSellerId?: string;
  }): Promise<{ url: string; fileKey: string; cloudinaryPublicId: string; duration?: number; format?: string; thumbnailUrl?: string }> {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (signatureResponse?.directUpload && signatureResponse.data) {
              ...
              try {
                ...
                if (isLastChunk) {
                  ...
                  const parallelPollPromise = new Promise<any>(async (pollResolve) => {
                    ...
                    try {
                      const verified = await api.pollVideoUploadVerification({
                        user,
                        publicId: fullPublicId,
                        signal: pollAbortCtrl.signal
                      });
                      if (verified && verified.isReady) {
                        pollResolve(verified);
                      }
                    } catch { }
                  });

                  chunkResponse = await Promise.race([chunkPromise, parallelPollPromise]);
                  pollAbortCtrl.abort();
                } else {
                  chunkResponse = await chunkPromise;
                }

                chunkSuccess = true;
                if (isLastChunk || chunkResponse?.secure_url || chunkResponse?.url || chunkResponse?.public_id) {
          ...
          const finalThumbnail = finalResult?.thumbnailUrl || finalUrl.replace('/video/upload/', '/video/upload/so_1.0/').replace(/\.[^/.]+$/, '.jpg');

          return resolve({
            url: finalUrl,
            fileKey: finalResult?.public_id || fullPublicId,
            cloudinaryPublicId: finalResult?.public_id || fullPublicId,
