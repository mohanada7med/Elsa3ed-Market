...
export const WAHFoodCard: React.FC<WAHFoodCardProps> = ({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="group relative flex flex-col rounded-[1.5rem] bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl border border-black/10 dark:border-white/10 hover:border-primary/40 dark:hover:border-primary/50 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer"
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-black/5 dark:bg-cream/5">
        <img
          src={image}
          alt={title}
        ...
        <div className="absolute bottom-2.5 right-2.5 z-10">
          <span className="bg-espresso/80 text-cream text-[10px] font-bold px-2.5 py-0.5 rounded-full backdrop-blur-md border border-white/10 flex items-center gap-1">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between relative bg-white/75 dark:bg-espresso-900/90">
        ...
        <div className="pt-3 mt-3 border-t border-black/10 dark:border-white/10 flex items-center justify-between text-xs font-bold text-primary dark:text-primary-hover relative z-10">
