...
export const WAHGovernorateCard: React.FC<WAHGovernorateCardProps> = ({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="group relative flex flex-col items-center justify-between p-3.5 sm:p-4 rounded-[1.5rem] bg-white/75 dark:bg-espresso-900/90 backdrop-blur-xl border border-black/10 dark:border-white/10 hover:border-primary/40 dark:hover:border-primary/50 shadow-lg hover:shadow-xl transition-all duration-250 cursor-pointer text-center select-none"
    >
      {/* Governorate Emblem / Photo Frame */}
      <div className="relative w-16 h-16 sm:w-18 sm:h-18 rounded-2xl overflow-hidden mb-3 border border-black/10 dark:border-white/10 group-hover:border-primary shadow-xs group-hover:scale-105 transition-all duration-300 shrink-0">
        <img
          src={image}
          alt={name}
